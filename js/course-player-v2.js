...
(function () {
  ...
  function updateTalk1VideoUI() {
    ...
    updateVideoControlsState({ skipTransport: true });
  }
  ...
  function forceTalk1MainPlayback(attempt) {
    var lesson = getCurrentLesson();
    if (!isTalk1Lesson(lesson) || talk1Phase !== 'main' || talk1MainPaused) return;
    attempt = attempt || 0;
    userStartedEmbed = true;
    ...
    if (bunnyPlayer) {
      try {
        applyTalk1Volume();
        if (bunnyPlayer.setCurrentTime && talk1EmbedCurrentSec < 1) {
  ...
  function forceUnifiedHtmlPlayback(attempt) {
    ...
    if (!el.heroVisual || !el.heroSource) return;
    attempt = attempt || 0;
    talk1MainPaused = false;
    hideTalk1EmbedOverlays();
    if (el.heroImage) {
    ...
    if (el.videoStage) {
      el.videoStage.classList.remove('is-embed-mode', 'is-embed-visible', 'is-embed-playing', 'is-image-mode');
    }

    var expectedSrc = encodeMediaPath(lesson.heroVisual.src);
    var currentSrc = el.heroSource.getAttribute('src') || '';
    if (!currentSrc || currentSrc.indexOf(lesson.heroVisual.src) === -1) {
      el.heroSource.src = expectedSrc;
      el.heroSource.type = videoMimeFromSrc(lesson.heroVisual.src);
      el.heroVisual.load();
    }
    ...
    el.heroVisual.addEventListener('loadeddata', function onReady() {
      el.heroVisual.removeEventListener('loadeddata', onReady);
      tryPlay();
    }, { once: true });
  ...
  function finishUnifiedHtmlVideo() {
    ...
    if (el.videoStage) {
      el.videoStage.classList.remove(
        'is-talk1-embed-booting',
        'is-talk1-embed-reveal',
  ...
  function updateVideoControlsState(options) {
    options = options || {};
    var lesson = getCurrentLesson();
  ...
  function setupBunnyPlayer() {
    ...
    bunnyPlayer.on('ready', function () {
      ...
      updateEmbedVideoUI();
      if (isTalk1Lesson(getCurrentLesson()) && talk1Phase === 'main' && userStartedEmbed && !talk1MainPaused) {
        talk1EmbedPendingMuted = false;
        embedPendingPlay = false;
        unmuteAndPlayEmbed();
        revealTalk1EmbedWhenPlaying();
  ...
  function playEmbedVideo() {
    userStartedEmbed = true;
    if (isTalk1Lesson(getCurrentLesson())) talk1MainPaused = false;
  ...
  function silenceAuxiliaryMedia() {
    ++splashSequenceId;
    clearSplashTimers();
    ...
    if (el.splashAudio) {
      ...
      try { el.splashAudio.load(); } catch (err) {}
    }
    if (el.outroAudio) {
      el.outroAudio.pause();
      el.outroAudio.removeAttribute('src');
      try { el.outroAudio.load(); } catch (err2) {}
    }
    if (el.audio) {
      el.audio.pause();
      el.audio.muted = true;
    ...
    pauseWelcomePromo();
    if (welcomePromoInstance && typeof welcomePromoInstance.destroy === 'function') {
      try { welcomePromoInstance.destroy(); } catch (err4) {}
  ...
  function playSplashAudioNow(fromStart, sequenceId) {
    if (!el.splashAudio || !el.splashAudio.src) return;
    var activeId = sequenceId || splashSequenceId;
    if (fromStart) el.splashAudio.currentTime = 0;

    function tryPlay() {
      if (activeId !== splashSequenceId) return;
      if (!el.splashAudio || !el.splashAudio.src) return;
    ...
    el.splashAudio.addEventListener('loadeddata', function onLoadedData() {
      ...
      if (activeId !== splashSequenceId) return;
      if (el.splashAudio && el.splashAudio.paused) tryPlay();
    }, { once: true });
  }
  ...
  function startIntroPlayback(sequenceId) {
    ...
    playSplashAudioNow(true, sequenceId);
  ...
  function forceHeroVideo(src, lesson) {
    ...
    if (el.heroImage) {
      el.heroImage.hidden = true;
      el.heroImage.setAttribute('hidden', '');
      el.heroImage.removeAttribute('src');
      el.heroImage.style.display = 'none';
    }
    el.heroVisual.hidden = false;
    el.heroVisual.classList.remove('is-splash-hidden', 'is-embed-preview');
    el.heroVisual.style.opacity = '1';
    el.heroVisual.style.visibility = 'visible';
    if (el.videoStage) {
